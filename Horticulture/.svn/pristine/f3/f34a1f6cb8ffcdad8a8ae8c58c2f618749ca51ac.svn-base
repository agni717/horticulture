								...
								<form id="frmAppRej" name="frmAppRej">
								  ...
								  <div class="remarks_container">
									...
									<div class="row">
                                      ...
                                      <div class="col-lg-4 mt-0">
										<h5 class="py-2 text-success">District Approval 2nd Installment: <?php echo $app_data->dist_second_installment_amount; ?></h5>
									  </div>
									</div>
									<div class="form-group form-row">
									  <div class="col-lg-4">
										<label class="col-form-label" id="remarks_label">TV No:</label>
										<input type="text" class="form-control" name="second_release_tv_no" id="second_release_tv_no" />
									  </div>
									  <div class="col-lg-4">
										<label class="col-form-label" id="remarks_label">Token No:</label>
										<input type="text" class="form-control" name="second_release_token_no" id="second_release_token_no" />
									  </div>
									  <div class="col-lg-4">
										<label class="col-form-label" id="remarks_label">Date:</label>
										
										  <div class="input-group date date-picker">
											<input type="text" class="form-control" name="second_release_token_date" id="second_release_token_date"/>
									  ...
									  <div class="col-lg-4">
										...
										<input type="text" class="form-control" name="second_release_rbi_transaction" id="second_release_rbi_transaction" />
									  </div>
									  <div class="col-lg-4">
										<label class="col-form-label" id="remarks_label">Date:</label>

										  <div class="input-group date date-picker">
											<input type="text" class="form-control" name="second_release_rbi_trans_date" id="second_release_rbi_trans_date"/>
											<span class="input-group-append">
											  <span class="input-group-text bg-light d-block">
												<i class="fa fa-calendar"></i>
											  </span>
									  ...
									  <div class="col-lg-4">
										<label class="col-form-label" id="remarks_label">2nd installment released by HQ:</label>
										<input type="text" class="form-control" name="hq_second_release_amount" id="hq_second_release_amount"  />
									  </div>                     
									</div>
									</div>
 ...
 $(function(){
  ...
  function lfn_app_rej_rev(act)
  {
   ...
   $('#btn_submit').on("click",function(event){
	  ...
	  var second_release_tv_no = $('#second_release_tv_no').val().trim();
      var second_release_token_no = $('#second_release_token_no').val().trim();
	  var second_release_token_date = $('#second_release_token_date').val().trim();
	  var second_release_rbi_transaction = $('#second_release_rbi_transaction').val().trim();
	  var second_release_rbi_trans_date = $('#second_release_rbi_trans_date').val().trim();
	  var hq_second_release_amount = $('#hq_second_release_amount').val().trim();
      var remarks = $("#remarks").val();
	  var app_remarks = $("#app_remarks").val();
      var error_flag = false;
      //alert('hhh');
	  if(cnt_status == "Approve"){
		  if(second_release_tv_no.length==0){
			 $("#second_release_tv_no").focus();
			 $('#ar_msg_cont').show().html('TV No cannot be blank');
			 setTimeout(function(){$('#ar_msg_cont').hide()}, 4000);
			 error_flag = true;
			 return false;
		  }
		  if(second_release_token_no.length==0){
			 $("#second_release_token_no").focus();
			 $('#ar_msg_cont').show().html('Token No cannot be blank');
			 setTimeout(function(){$('#ar_msg_cont').hide()}, 4000);
			 error_flag = true;
			 return false;
		  }
		  if(second_release_token_date.length==0){
			 $("#second_release_token_date").focus();
			 $('#ar_msg_cont').show().html('Token date cannot be blank');
			 setTimeout(function(){$('#ar_msg_cont').hide()}, 4000);
			 error_flag = true;
			 return false;
		  }
		  if(second_release_rbi_transaction.length==0){
			 $("#second_release_rbi_transaction").focus();
			 $('#ar_msg_cont').show().html('RBI Transaction cannot be blank');
			 setTimeout(function(){$('#ar_msg_cont').hide()}, 4000);
			 error_flag = true;
			 return false;
		  }
		  if(second_release_rbi_trans_date.length==0){
			 $("#second_release_rbi_trans_date").focus();
		  ...
		  if(hq_second_release_amount == ""){
		   	 $("#hq_second_release_amount").focus();
			 $('#ar_msg_cont').show().html('HQ release approval amount cannot be blank');
			 setTimeout(function(){$('#ar_msg_cont').hide()}, 4000);
			 error_flag = true;
			 return false;
		  }
		  if(!hq_second_release_amount.match(onlynumerics)){
				 $("#hq_second_release_amount").focus();
      ...
      if(error_flag == false){
         $.ajax({
                url: '<?php echo base_url('admin/hq_dashboard/ajx_submit_2nd_installment_release_by_hq') ?>',
                type: 'POST',
                data: formData, 
                contentType: false,
                cache: false,

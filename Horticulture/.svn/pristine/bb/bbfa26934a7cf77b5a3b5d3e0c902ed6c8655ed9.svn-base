...
class Userset extends Application_Controller {
	...
	public function update_alldocument_details(){
		if($_POST){
			...
			if ($this->form_validation->run()) {
					...
					if (isset($_FILES[$file_doc]) && !empty($_FILES[$file_doc])) {
						...
						if ($upload_info['error']) {
							...
							if($this->main_m->alldocfiles_insert_update($rowsets, $fileupid) == FALSE){
								...
								$tot_doc_submitted[] = $alldocs->doc_id;
							}
						}
					}else{
						if($has_yesno == "No"){
				...
				// echo '<pre>'; print_r($tot_doc_submitted); die;
				foreach($mandatoryDocIdArr as $docId){
					if(!in_array($docId, $tot_doc_submitted)){
						$error_section++;
						$error_received = $error_received.' - Upload all mandatory Documents.<br/>';
						break;
					}
				}
				...
				if($error_section == 1){
					...
					if ($this->main_m->submit_project_details($this->session->userdata('member_id'), $this->session->userdata('member_scm_id'), $row_arr) == true) {
						...
						/* ============================= */
							$remarks_arr = array(
								'application_id' => $projsets->application_id,
								'user_id' => $this->session->userdata('member_id'),
								'remarks' => 'New application submitted by an applicant.',
								'remark_stat' => 8,
								'add_datetime' => date('Y-m-d H:i:s')
							);
							if($this->main_m->add_remarks($remarks_arr) != false){
								echo json_encode(array('msg' => 1));
							} else {
								echo json_encode(array('msg' => 0, 'e_msg' => 'There have some DB Updation Problem, Try again.'));
							}
	...
	public function application_no_popup()
    {
        $sch_id = $data['sch_id'] = $this->session->userdata('member_scm_id'); 
        $user_id = $data['user_id'] = $this->session->userdata('member_id');
        $application_id = $data['application_id']= $this->main_m->get_application_id($user_id,$sch_id);
        $data['application_no'] = $application_no = $this->main_m->get_application_no($user_id,$sch_id);
        $this->load->view('main/citizen/application_no_popup',$data);
    }

	/* public function application_form_view_download($application_id)
    {
        if($application_id == "" || $application_id == NULL){
			redirect('dafault404');
		}
		...
		$this->data['document_upload_set'] = $this->main_m->get_document_upload_master_data_via_appid($application_id);
		$this->data['application_id'] = $application_id;
		$this->load->view('main/citizen/application_form_view_download',$this->data);
    } */
	...
	public function application_form_view_download($application_id){
		...
		$this->data['industry_data'] = $industry_data = $this->admin_m->get_industry_master_data('',$application_id);
		$user_details = $this->data['user_details'];
		/* ============================================= */
		...
		$title = "ESI (MB) SCHEME";//$advice_detail->advice_name;
		...
		$html_set = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
			...
			<body>
				<page size="A4">
					<table width="100%" border="0">
						<tr>
							<td width="79%" style="text-align:center;"><h2><strong>Application Form<br /> 
							West Bengal State Food Processing Incentive Scheme</strong><br />
							<h2><strong>Application No : </strong><label>'.$app_data->application_no.'</label></h2>
							</h2></td>
							<td width="21%"><strong>Applicant Photo :</strong><br /><br /><img src="'.base_url('uploads/applicant_photos/'.$app_data->image).'" width="150" height="180" /></td>
						</tr>
						<tr>
							<td></td>
							<td>&nbsp;</td>
					...
					<table>
						...
						<tr>
							<td colspan="3"><strong>Personal Description </strong></td>
						</tr>
						<tr>
							<td colspan="3">&nbsp;</td>
						</tr>
					...
					<table width="100%" border="0" cellpadding="5" class="box">
						<tr>
							<td><strong>Name Of Applicant :</strong> <label>'.$app_data->first_name.'</label></td>
							<td><strong>Mobile :</strong><label>'.$app_data->mobile.'</label> </td>
							<td><strong>Mail Id :</strong> <label>'.$app_data->email.'</label></td>
						</tr>
						<tr>
							<td><strong>District :</strong> <label>'.$app_data->district_name.'</label></td>
							<td><strong>Sub-Division :</strong>  <label>'.$app_data->subdiv_name.'</label></td>
							<td><strong>Block :</strong> <label>'.$app_data->block_name.'</label></td>
						</tr>
						<tr>
							<td><strong>Gram Panchayat :</strong> <label>'.$app_data->panchayat_name.'</label></td>
							<td><strong>Village/Town :</strong> <label>'.$app_data->village.'</label></td>
							<td><strong>Post Office :</strong> <label>'.$app_data->post_office.'</label></td>
						</tr>
						<tr>
							<td><strong>Pincode : </strong><label>'.$app_data->pincode.'</label></td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
					</table>
					<table>
						...
						<tr>
							<td colspan="3"><strong>Project Description </strong></td>
						</tr>
						<tr>
							<td colspan="3">&nbsp;</td>
						</tr>
					...
					<table width="100%" border="0" cellpadding="5" class="box">
						...
						<tr>
							<td colspan="3" style="text-align:center"><strong>Existing Industry 1</strong></td>
						</tr>
						<tr>
							<td><strong>Existing Industry Name : </strong> <label>asd</label></td>
							<td><strong>Existing Industry Location : </strong>  <label>ass </label></td>
							<td><strong>Product of Existing Industry :</strong> <label>ass </label></td>
						</tr>
						<tr>
							<td><strong>Address1 :</strong> <label>add 1</label></td>
							<td><strong>District :</strong> <label> Alipurduar</label></td>
							<td><strong>Sub-Division : </strong> <label>Alipurduar</label></td>
						</tr>
						<tr>
							<td><strong>Block  :</strong> <label>Alipurdur I Block</label></td>
							<td><strong>DGram Panchayat :</strong> <label> Banchukamari</label></td>
							<td><strong>Police Station :  </strong> <label>XTZ</label></td>
						</tr>
						<tr>
							<td><strong>Pincode : </strong><label>700092</label></td>
							<td>&nbsp;</td>
							<td>&nbsp;</td>
						</tr>
					</table>
					...
					<table width="100%" border="0" cellpadding="5" class="box">
						...
						<tr>
							<td width="36%"><strong>Name of the Project  : </strong> <label>asd</label></td>
							<td width="30%"><strong>Products to be manufactured  : </strong>  <label>ass </label></td>
							<td width="34%"><strong>Daag No. :</strong> <label>ass </label></td>
						</tr>
						<tr>
							<td><strong>Katian No.  :</strong> <label>add 1</label></td>
							<td><strong>Mouza No. :</strong>       <label> Alipurduar</label></td>
							<td><strong>J.L. No. :</strong>      <label>Alipurduar</label></td>
						</tr>
						<tr>
							<td><strong>Land Type :</strong>       <label>Alipurdur I Block</label></td>
							<td><strong>Current Status :</strong>       <label> Banchukamari</label></td>
							<td><strong>Proposed date of commencement commercial production :</strong>      <label>XTZ</label></td>
						</tr>
						<tr>
							<td><strong>Technology :</strong>      <label> Indigenous</label></td>
							<td><strong>Capacity of the Plant :</strong></td>
							<td><strong>Present Capacity :</strong></td>
						</tr>
						<tr>
							<td><strong>Proposed Capacity :</strong>     <label>32</label></td>
							<td></td>
							<td></td>
						</tr>
					</table>  
					<table>
						...
						<tr><td colspan="3">&nbsp;</td></tr>
						<tr>
						<td colspan="3"><strong>Term loan received from Bank </strong></td>
						</tr>
						<tr><td colspan="3">&nbsp;</td></tr>
					</table>
					<table width="100%" border="0" cellpadding="5" class="box">
						<tr><td colspan="3"></td></tr>
						<tr>
							<td width="36%"><strong>Name of Bank   : </strong> <label>Allahabad Bank</label></td>
							<td width="30%"><strong>Branch : </strong>  <label>BANKURA </label></td>
							<td width="34%"><strong>IFSC Code :</strong> <label>ALLA0210410</label></td>
						</tr>
						<tr>
							<td><strong>Account No   :</strong> <label>199091536147</label></td>
							<td><strong>PAN No : </strong>       <label> ABCD0176W3</label></td>
							<td><strong>Term loan sanction : </strong>  <label>11</label></td>
						</tr>
					</table> 
					<table>
						<tr>
							<td colspan="3"><strong><h2>Machinery Specification & Project Cost </h2></strong></td>
						</tr>
					</table>
					<table>
						<tr><td colspan="3">&nbsp;</td></tr>
						<tr>
							<td colspan="3"><strong>Plant & Machinery Specification1</strong></td>
						</tr>
						<tr><td colspan="3">&nbsp;</td></tr>
					</table>
					<table width="100%" border="0" cellpadding="5" class="box">
						<tr><td colspan="3"></td></tr>
						<tr>
							<td width="36%"><strong>Machine Name    : </strong> <label>AAAA</label></td>
							<td width="30%"><strong>Manufactirer Name :  </strong>  <label>SSS </label></td>
							<td width="34%"><strong>Serial No. : </strong> <label>123</label></td>
						</tr>
						<tr>
							<td><strong>Model No.    :</strong> <label> 222333</label></td>
							<td><strong>Machine Type : </strong>       <label>  Imported</label></td>
							<td><strong>Cost : </strong>  <label> 12.00</label></td>
						</tr>
						<tr>
							<td><strong>Capacity    :</strong> <label> 22</label></td>
							<td></td>
							<td></td>
						</tr>
					</table> 
					...
					<table width="100%" border="0" cellpadding="5" class="box">
						<tr><td colspan="3"></td></tr>
						<tr>
							<td width="36%"><strong>Land & Buiding Cost   : </strong> <label>AAAA</label></td>
							<td width="30%"><strong>Plant and Machineries Cost :  </strong>  <label>SSS </label></td>
							<td width="34%"><strong>Misc. Fixed Assets : </strong> <label>123</label></td>
						</tr>
						<tr>
							<td><strong>Preli. & Pre-Operative Expenses :</strong> <label> 222333</label></td>
							<td><strong>Contingencies etc  : </strong>       <label>  Imported</label></td>
							<td><strong>Working Capital Margin : </strong>  <label> 12.00</label></td>
						</tr>
						<tr>
							<td><strong>Others    :</strong> <label> 22</label></td>
							<td><strong>Total Project Cost :</strong> 123</td>
							<td></td>
						</tr>
					</table>
					<table>
						<tr><td colspan="3">&nbsp;</td></tr>
						<tr>
							<td colspan="3"><strong>Means of Finance</strong></td>
						</tr>
						<tr><td colspan="3">&nbsp;</td></tr>
					</table>
					<table width="100%" border="0" cellpadding="5" class="box">
						<tr><td colspan="3"></td></tr>
						<tr>
							<td width="36%"><strong>Promoters Contribution : </strong> <label>AAAA</label></td>
							<td width="30%"><strong>Term loan from bank / F.I. : </strong>  <label>SSS </label></td>
							<td width="34%"><strong>Others :</strong> <label>123</label></td>
						</tr>
					</table>
					<table width="100%" border="0" cellpadding="5" class="box">
						<tr><td colspan="3"></td></tr>
						<tr>
							<td width="36%"><strong>Working Capital  : </strong> <label>AAAA</label></td>
							<td width="30%"><strong>Subsidy claimed under WBFCIS - 2021 . : </strong>  <label>SSS </label></td>
						</tr>
					</table>  
					<table>
						<tr><td colspan="3">&nbsp;</td></tr>
						<tr>
							<td colspan="3"><strong>Proposed no of Employment Generated: Proposed no of Employment Generated (Indirect)</strong></td>
						</tr>
						<tr><td colspan="3">&nbsp;</td></tr>
					</table>
					<table width="100%" border="0" cellpadding="5" class="box">
						<tr><td colspan="3"></td></tr>
						<tr>
							<td width="36%"><strong>Male : </strong> <label>AAAA</label></td>
							<td width="30%"><strong>Female  : </strong>  <label>SSS </label></td>
						</tr>
					</table> 
					<table>
						<tr><td colspan="3" style="height:80px;">&nbsp;</td></tr>
						<tr>
							<td colspan="3"><strong>Proposed no of Employment Generated: Proposed no of Employment Generated (Direct)</strong></td>
						</tr>
						<tr><td colspan="3">&nbsp;</td></tr>
					</table>
					<table width="100%" border="0" cellpadding="5" class="box">
					...
					<table width="100%" border="0">
						...
						<tr>
							<td width="61%">Date : <label>___/___/_____</label></td>
							<td width="39%">Signature :<label>______________________________</label></td>
						</tr>
					</table>
				</page>
			</body>
		...
		ob_end_clean();
		$obj_pdf->writeHTML($html_set, true, false, true, false, '');
		$obj_pdf->Output('ESI-Referral-Slip-'.'subha'.'.pdf', 'I');
